...
#include "sd/random_auto_g_file_loader.cpp"
...
bool random_auto_g_file_loader();              // random g loader
millis_t millis_auto_g;                        // random g loader
int lastrandomNumber;                          // last randon number used 
int feedRate;
int oldFeedRate = 30000;
int currentFeedRate;
char autoname[18];
millis_t millis_dwpot;
int DwPot;
millis_t currentDwell=1;
bool DWcont = true;
int oldDwell= 0;
int timees;
bool dwselector = true;
bool isprinting = true;
int giratiempo =25;
int SandTableStart = 0;
int alternator = 0;
bool Feedstabilize = false;
...
void loop() {
  do {
    ...
    if(!card.isPrinting()) {
      if (millis() - millis_auto_g > 10000) {
        millis_auto_g = millis();
        if (IS_SD_PAUSED()) {
          ...
          if (dwselector){
            ...
            switch(SandTableStart){
              case 0:                                         // Se lanza Aut0.g
                    queue.inject(F("G28XY"));
                    SERIAL_ECHOLNPGM("Haciendo la jomacion para auto0");
                    //currentFeedRate = map(feedRate, 0, 1023, 1, 100);
                  // sprintf_P(autoname, PSTR("M220 S%s"), i16tostr3left(currentFeedRate));
                    //Feedstabilize = true;
              ...
              case 1:
                    //random_auto_g_file_loader(); 
                    //if(card.flag.sdprinting == false){                                         // Se lanza el primer Erraser
                      SandTableStart = 2;
                      SERIAL_ECHOLNPGM("iniciando el primer erraser");
                      random_auto_g_file_loader();             // random g loader
                    //}
              ...
              case 2:
                //SERIAL_ECHOLNPGM("Caso 1 del sandtablestart");

                switch(alternator){
                  case 1:
                      SERIAL_ECHOLNPGM("iniciando el patron");
                      random_auto_g_file_loader();             // random g loader
                  ...
                  case 0:
                    ...
                    //if (!card.isPrinting()) {
                      ...
                      timees = ((currentDwell) - (millis() - millis_dwpot))/1000;
                      if (timees != giratiempo){
                        SERIAL_ECHOLNPGM("T- ", timees);
                        giratiempo = timees;
                      ...
                      if (timees <= 0) {
                        SERIAL_ECHOLNPGM("Continuando ");
                        DWcont = true;
                        isprinting = true;
                        //queue.inject(F("G28XY"));
                        //SERIAL_ECHOLNPGM("Haciendo la jomacion");
                        random_auto_g_file_loader();
            ...
            //}
              ...
              //SERIAL_ECHOLNPGM("SandTable is Paused forever");
          }
        }
    ...
    if (DwPot >= oldDwell + 10 || DwPot <= oldDwell - 10 ){
        if (DwPot < 204){
            currentDwell = 5000;//5s
            SERIAL_ECHOLNPGM("Resuming in 5s");
            //millis_dwpot = millis();
            dwselector = true;
        } else if (DwPot < 408){
            currentDwell = 360000;//5min
            SERIAL_ECHOLNPGM("Resuming in 5MIN");
            //millis_dwpot = millis();
            dwselector = true;
        } else if (DwPot < 612){
            currentDwell = 3600000; //1h
            SERIAL_ECHOLNPGM("Resuming in 1H");
            //millis_dwpot = millis();
            dwselector = true;
        }else if (DwPot < 816){
            currentDwell = 43200000; //12h
            SERIAL_ECHOLNPGM("Resuming in 12H");
            //millis_dwpot = millis();
            dwselector = true;
        } else {
            dwselector = false;
            SERIAL_ECHOLNPGM("Paused forever");
        }
        oldDwell = DwPot;
    }
    ...
    //SERIAL_ECHOLNPGM("Feedrate: ", feedRate);

    if (feedRate != oldFeedRate){//feedRate >= oldFeedRate + 10 || feedRate <= oldFeedRate - 10 ){
      currentFeedRate = map(feedRate, 0, 1023, 1, 100);
        sprintf_P(autoname, PSTR("M220 S%s"), i16tostr3left(currentFeedRate));
        Feedstabilize = true;
      ...
      if(Feedstabilize){
        ...
        //SERIAL_ECHOLNPGM("Feedrate: ", currentFeedRate);
        Feedstabilize = Feedstabilize;
        Feedstabilize = false;
      }
